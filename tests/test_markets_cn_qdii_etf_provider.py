...
class CodeNormalizationTests(unittest.TestCase):
    def test_normalize_ts_code_infers_exchange_suffix(self):
        ...
        self.assertEqual(normalize_ts_code("501018"), "501018.SH")
        self.assertEqual(normalize_ts_code("520870"), "520870.SH")
...
class ProviderSnapshotTests(unittest.TestCase):
    ...
    def test_offline_price_snapshot_uses_latest_prior_dated_cache(self):
        with TemporaryDirectory() as tmp:
            cache = Path(tmp)
            pd.DataFrame(
                [
            ...
            prior = _daily_frame().copy()
            prior["ts_code"] = "501018.SH"
            prior["trade_date"] = prior["trade_date"].where(
                prior["trade_date"] != prior["trade_date"].max(),
                "20260721",
            )
            prior.to_csv(cache / "fund_daily_501018_SH_20260721.csv", index=False)
            client = FakeTushareClient()
            provider = CNQDIETFProvider(
                pro_client=client,
                cache_dir=cache,
            ...
            snapshot = provider.price_snapshot("501018.SH", as_of="2026-07-23")

        self.assertEqual(snapshot.trade_date, "2026-07-21")
        self.assertFalse(snapshot.paused)
        self.assertEqual(client.daily_calls, [])
        self.assertIn(
            "prior_cache_fallback",
            {row["status"] for row in provider._health if row["source"] == "fund_daily"},
        )

    def test_offline_price_snapshot_never_uses_future_dated_cache(self):
        with TemporaryDirectory() as tmp:
            cache = Path(tmp)
            pd.DataFrame(
                [
            ...
            future = _daily_frame().copy()
            future["ts_code"] = "501018.SH"
            future.to_csv(cache / "fund_daily_501018_SH_20260724.csv", index=False)
            provider = CNQDIETFProvider(
                pro_client=FakeTushareClient(),
                cache_dir=cache,
                offline=True,
            ...
            with self.assertRaises(CacheMiss):
                provider.price_snapshot("501018.SH", as_of="2026-07-23")

    def test_online_price_snapshot_refreshes_a_prior_dated_cache(self):
        with TemporaryDirectory() as tmp:
            cache = Path(tmp)
            pd.DataFrame(
                [
            ...
            prior_dates = pd.bdate_range("2023-07-17", "2026-07-15")
            prior = pd.DataFrame(
                {
                    "ts_code": "513100.SH",
                    "trade_date": prior_dates.strftime("%Y%m%d"),
                    "open": 1.0,
                    "high": 1.0,
                    "low": 1.0,
                    "close": 1.0,
            ...
            prior.to_csv(cache / "fund_daily_513100_SH_20260715.csv", index=False)
            client = FakeTushareClient()
            provider = CNQDIETFProvider(
                pro_client=client,
                cache_dir=cache,
            ...
            snapshot = provider.price_snapshot("513100.SH", as_of="2026-07-17")

        self.assertEqual(snapshot.trade_date, "2026-07-17")
        self.assertEqual(len(client.daily_calls), 1)
