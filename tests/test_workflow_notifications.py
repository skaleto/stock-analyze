...
def _seed_semantic_status(root: Path) -> None:
    report = root / "reports" / "intelligence" / "semantic_status_latest.json"
    report.parent.mkdir(parents=True, exist_ok=True)
    report.write_text(
        json.dumps(
            {
                "metadata": {"documents": 123, "new_documents": 123},
                "artifacts": {
                    "queued": 100,
                    "downloaded": 118,
                    "parsed": 110,
...
class WorkflowSummaryTests(unittest.TestCase):
    ...
    def test_daily_summary_adds_one_compact_semantic_status_line(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _seed_registry(root)
            _seed_semantic_status(root)
        ...
        expected = (
            "公告：新增 123｜PDF 118/123｜事件 42｜隔离 3｜"
            "失败 1｜待处理 286｜契约 a-share-announcement-v1"
        )
        self.assertEqual(text.count("公告："), 1)
        self.assertIn(expected, text)
        self.assertNotIn("市场情报:", text)

    def test_semantic_status_fallback_is_bounded_and_compact(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _seed_registry(root)
            report = (
            ...
            report.parent.mkdir(parents=True, exist_ok=True)
            report.write_bytes(b"{" + (b"x" * 1_100_000))

            text = build_workflow_summary(
                "daily", root, today_d=date(2026, 7, 13), target="2026-07-13"
            )

        self.assertIn("公告：状态暂不可用", text)
        self.assertNotIn("x" * 100, text)

    def test_weekly_summary_does_not_repeat_routine_intelligence_status(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _seed_registry(root)
            _seed_semantic_status(root)
    ...
    def test_weekly_summary_only_adds_material_qdii_research_alerts(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            events.write_text(
                "event_id,report_id,code,name,category,title,published_at,observed_at,effective_at,expires_at,event_type,severity,hard_block,clears_temporary_blocks,restriction_scope,confirmation_status,source_kind,valid_to,resume_of,source_url,raw_content_hash,parser_version\n"
                "AN1,AN1,513100.SH,纳指ETF,1,暂停申购,2026-07-10T00:00:00,2026-07-10T08:00:00,2026-07-10T00:00:00,2026-08-09T00:00:00,suspension,hard,True,False,purchase,confirmed,fund_manager,2026-08-09T00:00:00,,https://example.test/a,hash,v2\n",
                encoding="utf-8",
            )
...
class WorkflowNotificationDeliveryTests(unittest.TestCase):
    def test_require_complete_defers_without_sending_or_marking_ledger(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _seed_registry(root)
            for market, agent in (
    ...
    def test_require_complete_checks_runs_before_delivery_deduplication(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _seed_registry(root)
            for market, agent in (
            ...
            ledger_path = root / "data" / "notifications" / "workflow_sent.json"
                ...
                json.dumps(
                    {
                        ...
                        "sent": {
                            "daily:2026-07-13": {
                                "sent_at": "2026-07-13T19:30:00+08:00"
                            }
                        },
                    }
                ),
            ...
            self.assertEqual(list(ledger["sent"]), ["daily:2026-07-13"])

    def test_require_complete_sends_after_all_four_tasks_succeed(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _seed_registry(root)
            for market in MARKETS:
    ...
    def test_intelligence_uses_the_existing_daily_delivery_ledger(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _seed_registry(root)
            _seed_semantic_status(root)
            ...
            self.assertIn("公告：新增 123", card["elements"][0]["text"]["content"])
            ledger = json.loads(
                (root / "data" / "notifications" / "workflow_sent.json").read_text(
                    encoding="utf-8"
                )
