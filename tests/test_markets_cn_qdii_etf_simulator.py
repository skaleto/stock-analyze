...
class ETFSimulatorTests(unittest.TestCase):
    ...
    def test_generate_rebalance_orders_rounds_target_to_100_share_lot(self):
        ...
        self.assertTrue(all(order["liquidity_status"] == "available" for order in orders))
        self.assertTrue(all(order["participation_rate"] is not None for order in orders))
        self.assertTrue(all("impact_capped" in order for order in orders))
        self.assertTrue(all("avg_daily_amount" in order for order in orders))
    ...
    def test_config_rebalance_caps_shared_underlying_company_exposure(self):
        config = _config()
        config["accounts"][0]["cash"] = 100_000.0
        config["accounts"][0]["top_n"] = 2
        config["trading"] = {"max_single_weight": 0.80}
        ...
        weights = {order["code"]: order["target_weight"] for order in orders}
        measured = (
            weights.get("ETF-A.SH", 0.0) * 0.25
            + weights.get("ETF-B.SH", 0.0) * 0.20
        )
        self.assertLessEqual(measured, 0.10 + 1e-8)
        diagnostics = selection["scopes"]["us_exposure"]["optimizer_diagnostics"]
        self.assertEqual(diagnostics["company_exposure_cap"], 0.10)
        self.assertLessEqual(diagnostics["max_measured_company_exposure"], 0.10 + 1e-8)
