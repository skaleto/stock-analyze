...
class DailyDecisionCycleTests(unittest.TestCase):
    def test_offline_as_of_resolves_qdii_fund_daily_cache_date(self) -> None:
        with TemporaryDirectory() as tmp:
            cache = Path(tmp)
            (cache / "fund_daily_513100_SH_20260709.csv").touch()
            (cache / "fund_daily_159920_SZ_20260710.csv").touch()
            (cache / "fund_daily_invalid.csv").touch()

            resolved = cli._resolve_offline_as_of(cache)

        self.assertEqual(resolved, "2026-07-10")

    def test_offline_as_of_prefers_completed_market_snapshot(self) -> None:
        with TemporaryDirectory() as tmp:
            shared = Path(tmp) / "shared"
            cache = shared / "cache"
            cache.mkdir(parents=True)
            (cache / "fund_daily_159920_SZ_20260808.csv").touch()
            (shared / "market_snapshot_2026-08-07.json").write_text(
                "{}",
                encoding="utf-8",
            )
        ...
        self.assertEqual(resolved, "2026-08-07")
