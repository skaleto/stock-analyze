    ...
    if model_root.exists():
        for path in model_root.iterdir():
            ...
            if path.name.isdigit():
                ...
                continue
            model_dirs.extend(
                (path.name, horizon_dir)
                for horizon_dir in path.iterdir()
                if horizon_dir.is_dir() and horizon_dir.name.isdigit()
            )
    model_dirs.sort(key=lambda item: (int(item[1].name), item[0]))
    if not model_dirs:
        return {"status": "unavailable", "models": []}
    models: list[dict[str, Any]] = []
    latest_models: list[dict[str, Any]] = []
    for account_scope, horizon_dir in model_dirs:
        metadata_files = sorted(horizon_dir.rglob("*.metadata.json"))
        ...
        def health_payload(candidate: str) -> dict[str, Any]:
            ...
            payload["account_scope"] = str(
                payload.get("account_scope") or account_scope
            )
