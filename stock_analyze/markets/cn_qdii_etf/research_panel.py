...
from .catalog import build_membership_calendar
from .data_provider import MAX_NAV_AGE_DAYS
from .units import canonicalize_tushare_amount
...
def _load_universe(path: Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    catalog_rows = payload.get("catalog_rows") if isinstance(payload, dict) else None
    if isinstance(catalog_rows, list):
        for raw in catalog_rows:
            if not isinstance(raw, dict) or not (raw.get("code") or raw.get("ts_code")):
                continue
...
def build_research_panel(
    ...
    *,
    ...
    universe_rows, universe_payload = _load_universe(universe_file)
    default_observation = (
        universe_payload.get("observation_date")
        or universe_payload.get("first_seen_at")
        or universe_payload.get("as_of")
    )
    membership = build_membership_calendar(
        universe_rows,
        dates=pd.date_range(start_ts.normalize(), end_ts.normalize(), freq="D"),
        default_observation=default_observation,
    )
    membership_frame = membership.frame.copy()
    codes = sorted(set(membership_frame.get("code", pd.Series(dtype=str)).astype(str)))
    ...
    member_metadata = membership_frame.copy()
    if not member_metadata.empty:
        member_metadata["trade_date"] = pd.to_datetime(
            member_metadata.pop("universe_date"),
            errors="coerce",
        )
        merge_columns = [
            "trade_date",
            "code",
            *[key for key in metadata_keys if key in member_metadata.columns],
        ]
        member_metadata = member_metadata[merge_columns].drop_duplicates(
            ["trade_date", "code"],
            keep="last",
        )
    ...
    for code in codes:
        ...
        daily["code"] = code
        daily = canonicalize_tushare_amount(daily)
    ...
    if frame.empty and codes:
        raise ResearchPanelError("empty_panel")
    if not frame.empty:
        sort_columns = [
            column for column in ("trade_date", "scope", "code") if column in frame.columns
        ]
        frame = frame.sort_values(sort_columns).reset_index(drop=True)
        frame["trade_date"] = frame["trade_date"].dt.strftime("%Y-%m-%d")
    quality = str(membership.metadata.get("quality") or "unavailable")
    metadata = {
        "schema_version": 2,
        "source_contract": (
            "point-in-time catalog observations"
            if quality == "available"
            else "catalog history unavailable; post-hoc interval diagnostic only"
        ),
        "quality": quality,
        "universe_as_of": membership.metadata.get("universe_as_of"),
        "provenance": membership.metadata.get("provenance"),
        "survivorship_bias": bool(membership.metadata.get("survivorship_bias", True)),
        "unbiased_universe": bool(membership.metadata.get("unbiased_universe", False)),
        "quality_reasons": list(membership.metadata.get("quality_reasons", [])),
        ...
        "start": str(frame["trade_date"].min()) if not frame.empty else start_ts.strftime("%Y-%m-%d"),
        "end": str(frame["trade_date"].max()) if not frame.empty else end_ts.strftime("%Y-%m-%d"),
