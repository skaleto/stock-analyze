...
def _record_funnel(
    ...
    capped_codes = set(capped.get("code", pd.Series(dtype=str)).astype(str))
    factor_ready_codes = set(
        factor_ready.get("code", pd.Series(dtype=str)).astype(str)
    )
    candidate_evaluations: list[dict[str, Any]] = []
    for _, row in spot_df.iterrows():
        code = str(row.get("code") or "")
        rejection_reason = _risk_rejection_reason(row, filters)
        if rejection_reason is None and code not in capped_codes:
            rejection_reason = "candidate_cap"
        if rejection_reason is None and code not in factor_ready_codes:
            rejection_reason = "insufficient_factor_coverage"
        candidate_evaluations.append(
            {
                ...
                "active_hard_event": bool(row.get("active_hard_event", False)),
                "unconfirmed_hard_events": row.get(
                    "unconfirmed_fund_event_blocks", []
                ),
            }
        )
    recorder(
