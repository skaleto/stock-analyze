...
from .._pricing import (
    ...
    trailing_volatility as _trailing_volatility,
)
...
class CNQDIETFProvider:
    ...
    def _attach_event_states(
        ...
        for rows in scopes.values():
            for row in rows:
                ...
                row["unconfirmed_fund_event_blocks"] = (
                    state.get("unconfirmed_hard_events") or []
                )
    ...
    def _fund_daily(self, ts_code: str, as_of_key: str) -> pd.DataFrame:
        ...
        if fallback is None:
            prior = self._latest_dated_cache("fund_daily", ts_code, as_of_key)
            if prior is not None:
                prior_name, fallback = prior
                cache_status = "prior_cache_fallback"
                self.record_health(
                    "fund_daily",
                    cache_status,
                    f"{prior_name} requested={cache_name}",
                    rows=len(fallback),
                )
        if fallback is not None:
            complete = self._daily_covers_window(fallback, start, as_of_key)
            latest_trade_date = self._latest_trade_date(fallback)
            if (
                self.offline
                or (complete and latest_trade_date == as_of_key)
                or cache_name in self._daily_refresh_attempted
            ):
    ...
    def _latest_dated_cache(
        ...
        if self.cache_dir is None:
            return None
        safe_code = ts_code.replace(".", "_")
        prefix = f"{kind}_{safe_code}_"
        candidates: list[tuple[str, Path]] = []
        for path in self.cache_dir.glob(f"{prefix}*.csv"):
            suffix = path.stem.removeprefix(prefix)
            if len(suffix) == 8 and suffix.isdigit() and suffix <= as_of_key:
                candidates.append((suffix, path))
        for _suffix, path in sorted(candidates, reverse=True):
            try:
                frame = self._read_cache(path.name)
