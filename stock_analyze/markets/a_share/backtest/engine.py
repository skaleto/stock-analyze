...
def _build_pending_batch(signals: List[dict], overlay: dict, as_of: date,
    ...
    for account in overlay.get("accounts", []):
        ...
        benchmark_code = str(account.get("benchmark") or "000300.SH")
        normalized_benchmark = benchmark_code.split(".", 1)[0].zfill(6)
        if not benchmark_returns.empty:
            return_history = return_history.copy()
            return_history[normalized_benchmark] = benchmark_returns.reindex(
                return_history.index
            )
        ...
        target_weights = risk_adjusted_target_weights(
            ...
            benchmark_weights={normalized_benchmark: 1.0},
