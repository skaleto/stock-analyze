...
def _compute_group(group: pd.DataFrame) -> pd.DataFrame:
    ...
    if has_volume:
        ...
        for column in ("volume_ratio_5_20", "volume_zscore_20", "obv", "ad", "mfi_14"):
            result[column] = np.nan

    if "amount" in group.columns:
        amount = pd.to_numeric(group["amount"], errors="coerce")
        result["amount_ratio_5_20"] = amount.rolling(5).mean() / amount.rolling(20).mean()
    else:
...
def compute_technical_features(ohlcv: pd.DataFrame) -> pd.DataFrame:
    ...
    if "code" in ordered.columns:
        parts = [_compute_group(group) for _, group in ordered.groupby("code", sort=False, dropna=False)]
        featured = pd.concat(parts, axis=0)
    else:
        featured = _compute_group(ordered)
