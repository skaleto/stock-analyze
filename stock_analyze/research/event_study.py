...
def build_event_study(
    ...
    *,
    ...
    for horizon in sorted(pd.to_numeric(label_frame["horizon"], errors="coerce").dropna().astype(int).unique()):
        ...
        for event_name in sorted(event_frame["event"].dropna().astype(str).unique()):
            event_slice = event_frame.loc[event_frame["event"].eq(event_name)]
            joined = event_slice.merge(
                horizon_labels,
                on=["code", "trade_date"],
                how="inner",
            )
