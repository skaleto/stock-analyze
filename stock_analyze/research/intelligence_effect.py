...
def evaluate_intelligence_increment(
    ...
    *,
    ...
    feature_admission = _event_feature_admission(
        support=support,
        event_features=available_event_features,
    ...
    return {
        ...
        "point_in_time_contract": "canonical_event_available_at_v1",
        "decision_boundary": (
...
def _event_feature_admission(
    *,
    support: dict[str, Any],
    event_features: Sequence[str],
    candidate_selected_features: Sequence[str],
    deltas: dict[str, float],
    paired_returns: dict[str, Any],
    permutation_importance: dict[str, float | None],
) -> dict[str, Any]:
    selected = set(candidate_selected_features).intersection(event_features)
    positive_importance = any(
        value is not None and float(value) > 0.0
        for value in permutation_importance.values()
    )
    confidence_interval = paired_returns.get("bootstrap_95_ci")
    paired_lower = (
        float(confidence_interval[0])
        if isinstance(confidence_interval, list)
        and len(confidence_interval) == 2
        and _is_number(confidence_interval[0])
        else float("-inf")
    )
    checks = {
        "point_in_time_contract": True,
        "active_date_support": int(support.get("active_dates") or 0)
        >= int(support.get("minimum_active_dates") or 0),
        "active_row_support": int(support.get("active_rows") or 0)
        >= int(support.get("minimum_active_rows") or 0),
        "rank_ic_uplift": float(deltas.get("rank_ic", float("-inf"))) > 0.0,
        "net_excess_return_uplift": float(
            deltas.get("net_excess_return", float("-inf"))
        ) > 0.0,
        "drawdown_not_worse": float(
            deltas.get("max_drawdown", float("inf"))
        ) <= 0.0,
        "turnover_uplift_bounded": float(
            deltas.get("annual_turnover", float("inf"))
        ) <= 0.50,
        "paired_period_support": int(paired_returns.get("periods") or 0) >= 3,
        "paired_return_ci_positive": paired_lower > 0.0,
        "event_feature_selected": bool(selected),
        "positive_permutation_importance": positive_importance,
    }
    reasons = [name for name, passed in checks.items() if not passed]
    return {
        "status": "shadow_eligible" if not reasons else "research_only",
        "passed": not reasons,
        "maximum_consumer_status": "shadow",
        "checks": checks,
        "reasons": reasons,
        "selected_event_features": sorted(selected),
    }
