...
class ModelIdentity:
    ...
    def __post_init__(self) -> None:
        if not str(self.market).strip():
            raise ValueError("model_market")
        if not str(self.account_scope).strip():
            raise ValueError("model_account_scope")
        if self.horizon not in SUPPORTED_HORIZONS:
            raise ValueError("model_horizon")
