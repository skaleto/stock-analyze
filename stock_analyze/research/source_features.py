...
def build_fundamental_history(frames: Mapping[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    if not income.empty:
        ...
        income_keep = income[[
            "ts_code", "end_date", "ann_date", "revenue", "n_income", "operating_margin",
            "rd_intensity", "cost_growth", "operating_profit_growth",
        ]].rename(columns={"ann_date": "income_available_date"})
        history = history.merge(income_keep, on=["ts_code", "end_date"], how="left")
    ...
    if not cashflow.empty:
        ...
        cash_keep = cashflow[["ts_code", "end_date", "ann_date", "n_cashflow_act", "free_cashflow"]].rename(
            columns={"ann_date": "cash_available_date"}
        )
        history = history.merge(cash_keep, on=["ts_code", "end_date"], how="left")
    ...
    if not balance.empty:
        ...
        balance_keep = balance[["ts_code", "end_date", "ann_date", "total_assets"]].rename(
            columns={"ann_date": "balance_available_date"}
        )
        history = history.merge(balance_keep, on=["ts_code", "end_date"], how="left")
...
def attach_point_in_time_features(prices: pd.DataFrame, history: pd.DataFrame) -> pd.DataFrame:
    ...
    for code, rows in prices.assign(_row_order=np.arange(len(prices))).groupby("code", sort=False):
        left = rows.copy()
        left["_merge_date"] = pd.to_datetime(left["trade_date"].astype("string").str[:8], format="%Y%m%d", errors="coerce")
        right = history.loc[history["code"].astype("string") == str(code)].copy()
        if right.empty:
            for column in source_columns:
                left[column] = np.nan
            output.append(left)
            continue
        right["_merge_date"] = pd.to_datetime(right["available_date"].astype("string").str[:8], format="%Y%m%d", errors="coerce")
        right = right.dropna(subset=["_merge_date"]).drop_duplicates("_merge_date", keep="last")
        merged = pd.merge_asof(
            left.sort_values("_merge_date"),
            right[["_merge_date", *source_columns]].sort_values("_merge_date"),
            on="_merge_date",
            direction="backward",
        )
        output.append(merged)
    return pd.concat(output, ignore_index=True).sort_values("_row_order").drop(columns=["_row_order", "_merge_date"]).reset_index(drop=True)
