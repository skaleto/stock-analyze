...
from .feature_registry import DEFAULT_REGISTRY_HASH
...
class ModelBundle:
    ...
    linear_ranking_model: Ridge | None = None
...
def _select_features(
    ...
    *,
    ...
    feature_ranks = numeric.groupby(dates, sort=False).rank(pct=True, method="average")
    target_ranks = target.groupby(dates, sort=False).rank(pct=True, method="average")
    feature_centered = feature_ranks - feature_ranks.groupby(dates, sort=False).transform("mean")
    target_centered = target_ranks - target_ranks.groupby(dates, sort=False).transform("mean")
    ...
    feature_correlations = ranked.corr(method="pearson")
    selected: list[str] = []
...
def _portfolio_oos_metrics(
    ...
    *,
    ...
    curve = np.cumprod(1.0 + clipped)
    periods_per_year = 252.0 / horizon
    net_excess_return = (
        float(np.exp(np.log(curve[-1]) * periods_per_year / len(curve)) - 1.0)
        if curve[-1] > 0
        else -1.0
    )
...
def _activation_metrics(
    *,
    ...
    evaluation = pd.DataFrame(
        {
            ...
            "fold": pd.to_numeric(validation.get("_walk_forward_fold"), errors="coerce").fillna(-1).to_numpy(),
        }
    ...
    for seed_index in range(seed_ranking_predictions.shape[1]):
        ...
        seed_daily_ics = [
            value
            for indices in evaluation.groupby("trade_date", sort=True).indices.values()
            if (value := _spearman(
...
def _fit_components(
    ...
    *,
    ...
    train_return_y = pd.to_numeric(train["excess_return"], errors="coerce").fillna(0.0).to_numpy(dtype=float)
    ...
    linear_ranking = Ridge(alpha=10.0)
    linear_ranking.fit(scaler.transform(train_x), train_return_y)
    ...
    *,
    ...
    for split_number, split in enumerate(splits):
        ...
        try:
            ...
            selected_columns, selection_diagnostics = _select_features(
                fit_train,
                candidate_columns,
            )
    ...
    columns, deployment_selection = _select_features(
        train,
        candidate_columns,
    )
    ...
    point_in_time_audit = bool(all(audit_results) and deployment_audit)
