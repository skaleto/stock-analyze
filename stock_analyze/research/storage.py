...
class ResearchStore:
    ...
    def model_root(self, market: str, account_scope: str, horizon: int) -> Path:
        safe_market = self._safe_partition(market, "market")
        safe_scope = self._safe_partition(account_scope, "account_scope")
        return self.root / "models" / safe_market / safe_scope / str(int(horizon))
    ...
    def read_feature_snapshot(self, market: str, as_of: str) -> pd.DataFrame:
        frame = pd.read_parquet(
            self.feature_snapshot_path(market, as_of), dtype_backend="pyarrow"
        )
    ...
    def read_label_snapshot(self, market: str, as_of: str) -> pd.DataFrame:
        frame = pd.read_parquet(
            self.label_snapshot_path(market, as_of), dtype_backend="pyarrow"
        )
        return self._normalize_identifiers(frame)
    ...
    def _normalize_identifiers(frame: pd.DataFrame) -> pd.DataFrame:
        ...
        for column in normalized.columns:
            ...
            if isinstance(dtype, pd.ArrowDtype) and pd.api.types.is_float_dtype(dtype):
                normalized[column] = pd.to_numeric(
                    normalized[column], errors="coerce"
                ).astype("float32")
            elif isinstance(dtype, pd.ArrowDtype) and pd.api.types.is_integer_dtype(dtype):
                values = normalized[column].dropna()
                arrow_type = dtype.pyarrow_dtype
                if (
                    values.size
                    and arrow_type.bit_width > 32
                    and int(values.min()) >= INT32_MIN
                    and int(values.max()) <= INT32_MAX
                ):
                    normalized[column] = normalized[column].astype(
                        "int32[pyarrow]"
                    )
