...
def _attach_a_share(
    ...
    *,
    ...
    if index_weights is None and not weight_root.exists():
        return _unavailable(frame, "index_weight_history_missing")
    input_rows = len(frame)
    row_keys = pd.DataFrame({
        "_row_id": range(input_rows),
        "code": _code_key(frame["code"]),
        "trade_date": _date_key(frame["trade_date"]),
    })
    row_keys["_month"] = row_keys["trade_date"].str[:6]
    memberships: list[pd.DataFrame] = []
    missing_months: list[str] = []
    materialized = index_weights is not None
    normalized_weights = pd.DataFrame()
    if materialized:
        required = {"index_code", "con_code", "trade_date"}
        if index_weights is None or required.difference(index_weights.columns):
            return _unavailable(frame, "materialized_index_weight_schema")
        normalized_weights = index_weights.copy()
        normalized_weights["index_code"] = _code_key(normalized_weights["index_code"])
        normalized_weights["con_code"] = _code_key(normalized_weights["con_code"])
        normalized_weights["trade_date"] = _date_key(normalized_weights["trade_date"])
        valid = (
            normalized_weights["index_code"].str.fullmatch(r"\d{6}", na=False)
            & normalized_weights["con_code"].str.fullmatch(r"\d{6}", na=False)
            & normalized_weights["trade_date"].str.fullmatch(r"\d{8}", na=False)
        )
        if normalized_weights.empty or not valid.all():
            return _unavailable(frame, "materialized_index_weight_invalid")
    for account in accounts:
        prefix = str(account["benchmark_code"]).split(".")[0].zfill(6)
        account_weights = normalized_weights.loc[
            normalized_weights["index_code"].eq(prefix)
        ] if materialized else pd.DataFrame()
        snapshots = sorted(weight_root.glob(f"{prefix}_????-??.csv")) if not materialized else []
        parsed = (
            [
        ...
        for month in sorted(row_keys["_month"].dropna().unique()):
            if materialized:
                target_dates = row_keys.loc[
                    row_keys["_month"].eq(month), "trade_date"
                ]
                cutoff = str(target_dates.min())
                eligible_dates = sorted(
                    account_weights.loc[
                        account_weights["trade_date"].le(cutoff), "trade_date"
                    ].unique()
                )
                if not eligible_dates:
                    missing_months.append(f"{account['account_id']}:{month}")
                    continue
                snapshot_month = str(eligible_dates[-1])
                weights = account_weights.loc[
                    account_weights["trade_date"].eq(snapshot_month)
                ]
            else:
    ...
    selected = row_keys.merge(membership, on=["_month", "code"], how="inner")
    selected = (
        selected.sort_values(["_row_id", "_account_order"], kind="stable")
        .drop_duplicates("_row_id", keep="first")
        .reset_index(drop=True)
    )
    row_ids = selected["_row_id"].to_numpy(dtype="intp", copy=False)
    merged = frame.iloc[row_ids].reset_index(drop=True).copy()
    merged["code"] = selected["code"].to_numpy()
    ...
    return _decorate(
        merged,
        source=("materialized_index_weight" if materialized else "monthly_index_weight"),
