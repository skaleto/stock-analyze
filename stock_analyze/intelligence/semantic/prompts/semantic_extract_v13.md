# Provider-Neutral Semantic Extraction V13
...
Every `number` fact represents one scalar economic value.
Never place a range or multiple amounts into one scalar fact. Omit an ambiguous optional fact; if
an ambiguous value is required by the taxonomy, omit the event. Do not silently
choose the lower bound, upper bound, first amount, or total plus interest.

For `cash_per_share`, copy the full `每N股派M元` source phrase as `raw_value`
and cite it. For `stock_per_share` or a rights-offering ratio, likewise copy
the full `每N股送M股`, `每N股转增M股`, or `每N股配M股` phrase. The runner derives
the true per-share scalar; never emit only the visible numerator without its
share base.
