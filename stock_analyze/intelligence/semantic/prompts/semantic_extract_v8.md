# Provider-Neutral Semantic Extraction V8
...
such as `人民币万元`; never canonicalize it. A table unit header
and value cell may use separate evidence items. Omit absent optional facts and
never emit a null `raw_value` placeholder. Every emitted event must have
