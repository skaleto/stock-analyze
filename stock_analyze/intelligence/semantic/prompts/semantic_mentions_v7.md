# Announcement Event Mention Extraction V7
...
For every emitted mention, include every name in `required_all_of`, and include
at least one complete alternative from `required_one_of_sets`. Fields listed in
`dedupe_fields` are also required unless the frozen taxonomy omits them from
the template. Do not emit a secondary event type when its required source facts
are absent; one well-supported primary event is better than an unsupported
...
For shareholder changes, `holding_after` is the post-change number of shares,
not a percentage. Put a percentage only in `share_ratio`; if the template has
no separate post-change ratio field, omit that optional percentage. `action`
must state the holder's actual increase, decrease, acquisition, transfer, or
other change, not merely repeat the transaction method. When a table value is
a percentage and the column header supplies `%`, cite both the exact scalar
cell and the exact `%` header, and include `%` in `raw_value`.

For earnings forecasts and flashes, percentage change fields must include `%`.
When a table cell contains only a scalar and its column header supplies `%`,
cite both fragments and include `%` in `raw_value`; do not treat the scalar as
a unitless ratio.
