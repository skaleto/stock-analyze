# Provider-Neutral Semantic Extraction V11
...
If the source wording for an optional fact spans two or more chunks, omit that
optional fact unless the label chunk and the value chunk together meet the
typed evidence contract. Do not reconstruct a sentence from adjacent chunks.
For a required fact, use exact source fragments only when they independently
establish the required meaning and value; otherwise omit the event.
