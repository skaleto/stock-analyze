                ...
                errors.append(
                    {
                        ...
                        "retryable": bool(
                            getattr(repair_exc, "retryable", False)
                        ),
                        "terminal": False,
                        "validation_repair_attempted": True,
                    }
...
def _validate_provider_result(
    ...
    *,
    ...
    if bundle.schema_version == MENTION_SCHEMA_VERSION:
        ...
        if compilation.rejected_mentions or compilation.dropped_items:
            reasons = [
                f"{item.mention_id}:{','.join(item.reason_codes)}"
                for item in compilation.rejected_mentions
            ]
            if compilation.dropped_items:
                reasons.append(
                    f"dropped_items:{compilation.dropped_items}"
                )
            raise SemanticContractError(
    ...
    *,
    ...
    for ordinal, chunk in enumerate(chunks):
        ...
        if section == "document_metadata":
            ...
            priority = 1 if body_ordinal < 2 else 3
            body_ordinal += 1
        elif _usable_ir_value_node(node):
            priority = 2
        else:
            priority = 4
        prioritized.append((priority, ordinal, chunk))

    retained: list[dict[str, object]] = []
    selected_ids: set[str] = set()
    selected_value_ids: set[str] = set()
    for _, _, chunk in sorted(prioritized, key=lambda value: value[:2]):
        chunk_id = str(chunk.get("chunk_id") or "")
        node = ir_nodes.get(chunk_id)
        if (
