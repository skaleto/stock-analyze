...
def generate_rebalance_orders(
    ...
    execute_after = next_business_day(run_date)
...
def execute_due_orders(
    ...
    for batch in pending:
        ...
        for order in sells + buys:
            trade = execute_order(config, account, order, provider, batch.get("execute_after"), batch.get("account_id"))
            if trade:
...
def execute_order(
    ...
    price = price or safe_float(order.get("reference_price"))
    if price is None or price <= 0:
        return None
    ...
    if side == "sell":
        shares = min(abs(raw_delta), current_shares)
    else:
    ...
    return {
        "trade_date": trade_date or execute_after,
        "account_id": account_id,
        "code": code,
        "name": order.get("name", code),
        "side": side,
