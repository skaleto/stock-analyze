...
| 消息 | 时间 | 内容 |
| --- | --- | --- |
| 日报 | 四条正式日流水线全部完成后；21:30 兜底检查 | 两市场、两策略总净值、当日成交与异常 |
...
```text
stock-analyze-daily-summary.timer    Mon-Fri 21:30 Asia/Shanghai (fallback)
...
四个 daily worker 的 `OnSuccess` 都会触发 `stock-analyze-daily-finalize.service`。
finalizer 最多等待 20 分钟，并且只有当四份 `runs.csv` 都有同一目标日的成功记录时
才发送日报；未齐返回可恢复状态 `75`，不发消息也不写去重账本。21:30 timer 只是
进程意外中断时的兜底检查。周提醒在周六 10:00 和 10:15 两组任务之后发送；月提醒
在 09:00 月度评审之后发送。
