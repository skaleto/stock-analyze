...
for svc in deploy/systemd/stock-analyze-*.service; do
  scp -i ~/.ssh/<ssh-key-file> "$svc" \
    root@120.55.188.242:/etc/systemd/system/"$(basename "$svc")"
done

ssh -i ~/.ssh/<ssh-key-file> root@120.55.188.242 "systemctl daemon-reload && systemctl list-timers 'stock-analyze-*' --no-pager | head -10"
```
...
```bash
ssh -i ~/.ssh/<ssh-key-file> root@120.55.188.242 "cd /opt/stock-analyze/app && bash scripts/migrate-phase-1-paths.sh"
```
...
rsync -avz -e "ssh -i ~/.ssh/<ssh-key-file>" \
  scripts/migrate-phase-1-paths.sh \
