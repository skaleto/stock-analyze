...
| LLM 情感因子 | ⚠️ MVP，**broadcast 标量对排名零影响**（见 A.2 P2） | **零 alpha** |
...
#### 🔴 P2：情感因子 MVP 是横截面 no-op —— 每周占用 operator 时间，产生零 alpha
...
| **4** | P2 情感因子去留 | 中（纯成本） | 决策为主 | **先讨论**（D.2） |
