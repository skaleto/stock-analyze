...
```mermaid
flowchart TD
    A[五个时点：官方资讯增量采集] --> B[18:30：行情与基础数据同步]
    ...
    I --> K{四份正式 runs.csv 均成功？}
    K -->|是| L[飞书每日摘要]
    K -->|否| M[等待或在 21:30 兜底复查]
```
...
正式模拟账户的 HS300、ZZ500 与跨境 ETF 账户范围保持锁定。研究侧另有独立目录：A 股保存全部在市主表，并把 HS300、ZZ500、CSI1000 作为叠加范围；每条记录带行业、板块、上市日期、最新总市值/流通市值日期和固定市值分层。三条指数共有 1,800 只成员，而当前 A 股主表约 5,549 只；其余约 3,749 只（包括小盘、微盘）可在 Dashboard 的“全市场 A 股”范围中检索，但不会进入正式账户。基金同时保存场内基金主表与场外基金主表。场内记录只作为研究候选，场外记录明确标为“非交易研究对照”，二者均不会进入订单或正式账户。

目录与明细分开落盘：`data/research/universe_catalogs/` 保存可分页检索的主数据；`data/research/a_share_prices/v1/` 保存 CSI1000 的研究专用三年日线；`data/research/otc_fund_nav/v1/` 保存纳斯达克 100 与标普 500 对应场外基金的复权净值。Dashboard 只读这些产物：场外基金展示净值曲线、区间收益、波动和最大回撤，不会将净值伪造成 K 线。
...
当前版本定义了 16 个 systemd timer，ECS 实际启用和加载数量以运行中心的实时读数为准。正式日链遵循“先数据、再研究、再四账户、最后完整性门禁与通知”；模型迭代在 4/4 完成后运行，公告回填在关键窗口外运行，二者失败都不会阻断正式纸面账户。
...
flowchart TD
    ...
    R --> W0[每周六]
    R --> M0[每月 1 日]
    D0 --> D1[五个时点：公告元数据增量采集]
    D0 --> D1A[约每 20 分钟：低优先级 PDF 与解析回填]
    D0 --> D2[18:30：行情与不可变研究快照]
    D2 --> D3[研究、预测与四个正式账户]
    D3 --> D4{四份正式账本均成功？}
    D4 -->|是| D5[立即发送飞书每日摘要]
    D4 -->|是| D5A[候选模型迭代]
    D4 -->|否| D6[21:30 兜底复查，仍不完整则不发送]
    D0 --> D7[22:10：语义抽取与确定性校验]
    D0 --> D8[00:30：近两日公告对账]
    R --> Q0[周日 03:15：全库情报质量报告]
    W0 --> W1[10:00：A 股周度材料]
...
| 周期 | 时间 | 主要动作 | 是否需要你介入 |
| --- | --- | --- | --- |
| **资讯** | 交易日 09:30、12:30、16:30、21:45、23:30 | 抓取官方政策、基金公告和 A 股公司公告元数据并去重 | 否 |
| **公告产物** | 约每 20 分钟，17:45-21:30 暂停 | 有界续跑 PDF 下载、文本优先解析、表格和必要 OCR 队列，不调用语义模型 | 否 |
| **交易日主链** | 周一至周五 18:30 | 同步数据、研究、预测并执行四个纸面账户 | 否 |
| **每日摘要** | 四账户完成后；21:30 兜底 | 仅在当日四份运行账本均成功后发送整体结果和关键异常 | 仅异常时查看 |
| **公告对账** | 每天 00:30 | 对账近两日目录，补下载并小批解析 | 仅异常时查看 |
| **语义抽取** | 周一至周五 22:10 | 使用当前生产执行器处理有界队列，经过确定性校验后持久化结构化事件 | 仅隔离队列持续堆积时查看 |
| **情报质量** | 周日 03:15 | 低峰扫描全库，刷新覆盖、失败分层和一致性报告 | 仅异常时查看 |
...
| **运维** | 版本定义的 15 个 timer 按依赖完成，Dashboard 新鲜，飞书消息少而完整 | 线上缺少计划 timer、同一错误重复告警、日报缺失、接口整体阻塞或任务静默失败 |
