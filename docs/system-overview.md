...
flowchart TD
    ...
    D0 --> D1A[每 10 分钟：PDF 下载与解析回填]
    D1 --> D2[18:30：数据、研究、模型、四账户执行]
    D1A --> D2
    D2 --> D3[正式账户与候选模型并行，互不阻断]
    D3 --> D4[19:30：飞书每日摘要]
    D4 --> D5[20:30：公告对账与质量报告]
    D5 --> D6[22:10：语义抽取与确定性校验]
    W0 --> W1[10:00：A 股周度材料]
...
| 周期 | 时间 | 主要动作 | 是否需要你介入 |
| --- | --- | --- | --- |
| **资讯** | 交易日 09:30、12:30、16:30、20:30、23:30 | 抓取官方政策、基金公告和 A 股公司公告元数据并去重 | 否 |
| **公告产物** | 每 10 分钟，22 点维护窗口除外 | 有界续跑 PDF 下载、文本优先解析、表格和必要 OCR 队列，不调用语义模型 | 否 |
| **交易日主链** | 周一至周五 18:30 | 同步数据、研究、预测并执行四个纸面账户 | 否 |
| **每日摘要** | 周一至周五 19:30 | 发送整体结果、关键异常和最多 3 条最高置信度预测 | 仅异常时查看 |
| **公告对账** | 每天 20:30 | 对账近两日目录，刷新因子快照和质量报告 | 仅异常时查看 |
