...
expected=(
  ...
  stock-analyze-tabular-forward.timer
...
for unit in "${disabled[@]}"; do
  if systemctl is-enabled --quiet "$unit" 2>/dev/null; then
    echo "ERROR: timer must remain disabled: $unit" >&2
    exit 1
  fi
done

systemctl list-timers --all 'stock-analyze-*' --no-pager
echo "OK: scheduled timers are enabled and pipeline-triggered/retired timers are disabled."

# A oneshot timer can stay enabled while its child service has been failing.
# Inspect the last recorded service result as a separate business-health gate.
health_services=(
...
for unit in "${health_services[@]}"; do
  ...
  if [[ "$result" == "success" && "$exit_status" == "75" ]]; then
    case "$unit" in
      stock-analyze-daily-finalize.service)
        echo "INFO: $unit deferred because the four formal daily ledgers were not complete."
        continue
        ;;
      stock-analyze-intelligence.service|stock-analyze-intelligence-artifact-backfill.service|stock-analyze-intelligence-semantic.service|stock-analyze-intelligence-quality.service)
  ...
  if [[ -n "$result" && "$result" != "success" ]] || [[ -n "$exit_status" && "$exit_status" != "0" ]]; then
    echo "ERROR: $unit last result=${result:-unknown}, exit=${exit_status:-unknown}" >&2
    exit 1
  fi
done
