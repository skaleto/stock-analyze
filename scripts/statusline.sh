...
SIG_FILE="$(first_existing_file data/a_share/claude/daily_nav.csv)"
...
SYNC_FILE="$(first_existing_file data/a_share/claude/daily_nav.csv)"
...
# Compose one-line
echo "A股 ${SIG:-no-data} | 稳健防守 ${CL_NAV} 待执行 ${CL_PEND} | 趋势进攻 ${CO_NAV} 待执行 ${CO_PEND}${FRESHNESS:+ | $FRESHNESS}"
