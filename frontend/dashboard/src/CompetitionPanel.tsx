...
import { ArrowUpRight, Building2, GitCompareArrows, Layers3, Radar, ShieldCheck, Split } from "lucide-react";
    ...
    <section className="competition-panel terminal-section" role="region" aria-label="双策略竞技场">
      <header className="competition-heading">
        ...
        <div className="divergence-strip" aria-label="策略差异指标">
          ...
          <div title="综合持仓、收益、每日决策、因子暴露与换手风格">
            <Radar size={15} aria-hidden="true" />
            <span>综合差异度</span>
            <strong>
              {comparison.pair.distinctness?.distinctness_score == null
                ? "样本积累中"
                : formatPercent(comparison.pair.distinctness.distinctness_score)}
            </strong>
          </div>
        </div>
      </header>
