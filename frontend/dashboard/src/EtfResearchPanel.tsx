...
function ShadowView({ research }: { research?: QDIIResearch }) {
  ...
  return <div className="research-view research-shadow-view">
    ...
    <div className="research-table-wrap"><table className="research-table"><thead><tr><th>范围</th><th>模型</th><th>累计</th><th>Sharpe</th><th>回撤</th><th>状态</th></tr></thead><tbody>{rows.map((row, index) => <tr key={`${row.scope}-${index}`}><td><b>{scopeLabels[row.scope ?? ""] ?? row.scope}</b><small>{row.asset_class}</small></td><td>{row.factor_model}</td><td className={(row.cumulative_return ?? 0) >= 0 ? "positive" : "negative"}>{formatPercent(row.cumulative_return)}</td><td>{row.sharpe_ratio?.toFixed(2) ?? "-"}</td><td className="negative">{formatPercent(row.max_drawdown)}</td><td><span className={`research-state state-${row.promotion_status}`}>{statusLabel(row.promotion_status)}</span></td></tr>)}</tbody></table>{rows.length === 0 ? <p className="terminal-empty">等待每周影子研究生成净值</p> : null}</div>
