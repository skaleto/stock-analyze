...
| --- | --- | --- |
| Intelligence | Every 30 minutes | Collect official policy/announcement metadata and extract auditable events |
| Daily | Mon-Fri 18:30 | Fetch both markets, build research, predict, run model iteration, then execute/value/decide for all four accounts |
...
| Model training | Day 1 02:30 | Train Challenger models and run automated promotion gates |
...
then performs 每日收盘决策 and replaces the next-session target from the latest close. `run-weekly`
refreshes diagnostics, reports, dashboards, and briefings. 周任务不生成订单。
QDII daily services have no fixed timer; the shared research service starts all
four daily services only after data and prediction work succeeds.
