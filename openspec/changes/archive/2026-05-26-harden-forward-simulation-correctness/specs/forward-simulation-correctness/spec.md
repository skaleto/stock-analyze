...
### Requirement: Optimistic Reference-Price Fallback

Optimistic reference-price fallback for normal simulated execution SHALL be removed; missing quotes leave the order pending instead.

#### Scenario: Missing Quote Used To Force-Fill
- **GIVEN** an order is due but no market quote is visible
- **WHEN** execution runs under the old behavior
- **THEN** the system previously forced a fill at the order's reference price
- **AND** that fallback is now removed; the order remains pending with `unfilled_reason="execution_quote_missing"`
